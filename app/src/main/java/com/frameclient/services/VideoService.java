                    ...
                    if (size > 100) {
                        ...
                        while (!h264_queue.isEmpty()) {
                            ...
                            if (frame != null) {
                                ...
                                if (frame.info.frametype == 0) {
                                    ...
                                    //Log.v(TAG,"len:"+len+"count: "+count + " data[0]" + data_array[0]+" data[1]" + 
                                    // data_array[1]+" data[2]" + data_array[2]+" data[3]" + data_array[3]+" data[4]"
                                    // + data_array[4]);
                                    if (count > 0) {
                                        //Log.i(TAG,"decode "+frame.info.frameId + "  frame SoftResource.bmp_queue
                                        // .size() = "+SoftResource.bmp_queue.size());
                                        ...
                                        Log.i(TAG, "decode " + frame.info.frameId + " failue queue size  " +
                                                h264_queue.size());
                                    }
                                }
                            }
                        ...
                        if (!h264_queue.isEmpty()) {
                            ...
                            if (frame != null) {
                                ...
                                //Log.v(TAG,"len: "+frame.info.datalen+" frameId: "+frame.info.frameId + " data[0]" +
                                // frame.data[0]+" data[1]" + frame.data[1]+" data[2]" + frame.data[2]+" data[3]" + 
                                // frame.data[3]+" data[4]" + frame.data[4]);
                                if (mPixel == null)
                                    return;
                                ...
                                // data_array[4]);
                                if (count > 0) {
                                    //Log.i(TAG,"decode "+frame.info.frameId + "  frame SoftResource.bmp_queue.size()
                                    // = "+SoftResource.bmp_queue.size());
                                    ...
                                    Log.i(TAG, "decode " + frame.info.frameId + " failue queue size  " + h264_queue
                                            .size() + " datalen = " + frame.info.datalen);
                                }
                            ...
                            if (key.isReadable()) {
                                ...
                                switch (opera) {
	                      			...
	                      			case 1: // login
                        			{
										...
										{											
								 			...
								 			if(res < 0)
								 			{
								 				...
								 				res = tcp.connect_sever(encoder_ip, SoftResource
								 				.videostream_socket_port);
								 			}
								 			else
								 			{
								 				String [] str = null;  
                                        ...
                                        switch (type1) {
                                            ...
                                            case 1: {
                                                if (type2 == 0) {
                                                    ...
                                                    bitmap = Bitmap.createBitmap(video_info.width, video_info.height,
                                                            Config.RGB_565);
                                                    buffer = ByteBuffer.wrap(mPixel);
                                                    decoder.init(video_info.vidoe_codec_id, video_info.width,
                                                            video_info.height, video_info.fps, video_info.bit_rate,
                                                            video_info.gop_size > 0 ? video_info.gop_size : 100);
                                                    Log.v(TAG, "10010 rece video stream data header: " + str);
                                                    break;

                                                } else if (type2 == 1) {
                                                    String str = new String(data_array);
                                                    str.replace("bit_rate=\"0\"", "bit_rate=\"500000\"");
                                                    video_info = tcp.parseVideoInfo(data_array);
                                                    mPixel = new byte[video_info.width * video_info.height * 2];
                                                    bitmap = Bitmap.createBitmap(video_info.width, video_info.height,
                                                            Config.RGB_565);
                                            ...
                                            case 2: {
                                                ...
                                                //Log.v(TAG,"rece the "+index+" frame data len "+frame.info.datalen +
                                                // " all buf "+data_buf_len);
                                    ...
                                    case 216: //frame info data
                                    {
                                        ...
                                        video_info = tcp.parseVideoInfo(data_array);
                                        mPixel = new byte[video_info.width * video_info.height * 2];
                                        bitmap = Bitmap.createBitmap(video_info.width, video_info.height, Config
                                                .RGB_565);
                                        buffer = ByteBuffer.wrap(mPixel);
                                        decoder.init(video_info.vidoe_codec_id, video_info.width, video_info.height, 
                                                video_info.fps, video_info.bit_rate, video_info.gop_size > 0 ? 
                                                        video_info.gop_size : 100);
